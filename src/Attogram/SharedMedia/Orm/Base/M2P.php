...
/**
 * Base class that represents a row from the 'm2p' table.
 *
 *
 *
 * @package    propel.generator.Attogram.SharedMedia.Orm.Base
 */
abstract class M2P implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Attogram\\SharedMedia\\Orm\\Map\\M2PTableMap';
    ...
    /**
     * Initializes internal state of Attogram\SharedMedia\Orm\Base\M2P object.
     */
    ...
    /**
     * Compares this with another <code>M2P</code> instance.  If
     * <code>obj</code> is an instance of <code>M2P</code>, delegates to
     * <code>equals(M2P)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|M2P The current object, for fluid interface
     */
     ...
     * @return $this|\Attogram\SharedMedia\Orm\M2P The current object (for fluent API support)
     */
    public function setMediaId($v)
    {
        ...
        if ($this->media_id !== $v) {
            $this->media_id = $v;
            $this->modifiedColumns[M2PTableMap::COL_MEDIA_ID] = true;
        }
    ...
    public function setPageId($v)
    {
        ...
        if ($this->page_id !== $v) {
            $this->page_id = $v;
            $this->modifiedColumns[M2PTableMap::COL_PAGE_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : M2PTableMap::translateFieldName('MediaId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->media_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : M2PTableMap::translateFieldName('PageId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = M2PTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Attogram\\SharedMedia\\Orm\\M2P'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildM2PQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
    ...
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildM2PQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                M2PTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(M2PTableMap::COL_MEDIA_ID)) {
            $modifiedColumns[':p' . $index++]  = 'media_id';
        }
        if ($this->isColumnModified(M2PTableMap::COL_PAGE_ID)) {
            $modifiedColumns[':p' . $index++]  = 'page_id';
        }

        $sql = sprintf(
            'INSERT INTO m2p (%s) VALUES (%s)',
    ...
    {
        $pos = M2PTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['M2P'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['M2P'][$this->hashCode()] = true;
        $keys = M2PTableMap::getFieldNames($keyType);
        $result = array(
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\Attogram\SharedMedia\Orm\M2P
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Attogram\SharedMedia\Orm\M2P The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(M2PTableMap::COL_MEDIA_ID)) {
            $criteria->add(M2PTableMap::COL_MEDIA_ID, $this->media_id);
        }
        if ($this->isColumnModified(M2PTableMap::COL_PAGE_ID)) {
            $criteria->add(M2PTableMap::COL_PAGE_ID, $this->page_id);
        }
    ...
    public function hashCode()
    {
        ...
        //relation m2p_fk_32dfd1 to table media
        if ($this->aMedia && $hash = spl_object_hash($this->aMedia)) {
            $primaryKeyFKs[] = $hash;
        } else {
            $validPrimaryKeyFKs = false;
        }

        //relation m2p_fk_c9a38a to table page
    ...
    {
        $pks = array();
        $pks[0] = $this->getMediaId();
        $pks[1] = $this->getPageId();
    ...
    {
        $this->setMediaId($keys[0]);
        $this->setPageId($keys[1]);
    }
    ...
    {
        return (null === $this->getMediaId()) && (null === $this->getPageId());
    }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Attogram\SharedMedia\Orm\M2P (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Attogram\SharedMedia\Orm\M2P Clone of current object.
     * @throws PropelException
     */
    ...
    {
        if ($this->aMedia === null && ($this->media_id != 0)) {
            $this->aMedia = ChildMediaQuery::create()->findPk($this->media_id, $con);
    ...
    {
        if ($this->aPage === null && ($this->page_id != 0)) {
            $this->aPage = ChildPageQuery::create()->findPk($this->page_id, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
    ...
    {
        if (null !== $this->aMedia) {
            $this->aMedia->removeM2P($this);
        }
        if (null !== $this->aPage) {
            $this->aPage->removeM2P($this);
        }
        $this->media_id = null;
        $this->page_id = null;
        $this->alreadyInSave = false;
    ...
    {
        return (string) $this->exportTo(M2PTableMap::DEFAULT_STRING_FORMAT);
    }
