...
abstract class Page implements ActiveRecordInterface
{
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : PageTableMap::translateFieldName('Title', TableMap::TYPE_PHPNAME, $indexType)];
            $this->title = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : PageTableMap::translateFieldName('Displaytitle', TableMap::TYPE_PHPNAME, $indexType)];
            $this->displaytitle = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : PageTableMap::translateFieldName('PageImageFree', TableMap::TYPE_PHPNAME, $indexType)];
            $this->page_image_free = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : PageTableMap::translateFieldName('WikibaseItem', TableMap::TYPE_PHPNAME, $indexType)];
            $this->wikibase_item = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : PageTableMap::translateFieldName('Disambiguation', TableMap::TYPE_PHPNAME, $indexType)];
            $this->disambiguation = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 7 + $startcol : PageTableMap::translateFieldName('Defaultsort', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 8; // 8 = PageTableMap::NUM_HYDRATE_COLUMNS.
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            ...
            $keys[3] => $this->getDisplaytitle(),
            $keys[4] => $this->getPageImageFree(),
            $keys[5] => $this->getWikibaseItem(),
            $keys[6] => $this->getDisambiguation(),
            $keys[7] => $this->getDefaultsort(),
        );
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[3], $arr)) {
            $this->setDisplaytitle($arr[$keys[3]]);
        }
        if (array_key_exists($keys[4], $arr)) {
            $this->setPageImageFree($arr[$keys[4]]);
        }
        if (array_key_exists($keys[5], $arr)) {
            $this->setWikibaseItem($arr[$keys[5]]);
        }
        if (array_key_exists($keys[6], $arr)) {
            $this->setDisambiguation($arr[$keys[6]]);
        }
        if (array_key_exists($keys[7], $arr)) {
            $this->setDefaultsort($arr[$keys[7]]);
        }
    }
